...
async function fetchData() {
  try {
    const response = await fetch("./states_all.csv");
    const text = await response.text();
    return Papa.parse(text, { header: true, dynamicTyping: true }).data;
  } catch (error) {
    console.error("Error loading data:", error);
    throw new Error(
      'Failed to load CSV file. Make sure "small.csv" is in the same directory as this HTML file.'
    );
  }
}
...
async function createVisualizations() {
  ...
  Plotly.newPlot("testScoreTrends", [scoresTrends], {
    ...
    scene: {
      xaxis: { title: "Math Score" },
      yaxis: { title: "Reading Score" },
      zaxis: { title: "Year", range: [1950, 2020] },
    },
  });
  ...
  const fundingFlowData = {
    ...
    link: {
      ...
      value: data
        .map((d) => [
          parseFloat(d.FEDERAL_REVENUE) * 0.6 || 0,
          parseFloat(d.STATE_REVENUE) * 0.6 || 0,
          parseFloat(d.LOCAL_REVENUE) * 0.6 || 0,
          parseFloat(d.FEDERAL_REVENUE) * 0.3 || 0,
          parseFloat(d.STATE_REVENUE) * 0.3 || 0,
          parseFloat(d.LOCAL_REVENUE) * 0.3 || 0,
          parseFloat(d.FEDERAL_REVENUE) * 0.05 || 0,
          parseFloat(d.STATE_REVENUE) * 0.05 || 0,
          parseFloat(d.LOCAL_REVENUE) * 0.05 || 0,
          parseFloat(d.FEDERAL_REVENUE) * 0.05 || 0,
          parseFloat(d.STATE_REVENUE) * 0.05 || 0,
          parseFloat(d.LOCAL_REVENUE) * 0.05 || 0,
        ])
  ...
  // 12. State Revenue Trends (Line Race Chart)
  const revenueRaceData = states.map((state) => ({
    x: years,
    y: years.map((year) => {
      const entry = data.find((d) => d.STATE === state && d.YEAR === year);
      return entry ? parseFloat(entry.TOTAL_REVENUE) : null;
    }),
    name: state,
    mode: "lines",
    type: "scatter",
  ...
  Plotly.newPlot("revenueRace", revenueRaceData, {
    title: "State Revenue Trends",
    xaxis: { title: "Year" },
    yaxis: { title: "Total Revenue" },
    updatemenus: [
      {
  ...
  const mathScoresMapData = [
    {
      ...
      colorscale: "Viridis",
      colorbar: { title: "4th Grade Math Score" },
      name: "4th Grade",
    },
    {
      type: "choropleth",
      locationmode: "USA-states",
      locations: latestData.map((d) => d.STATE),
      z: latestData.map((d) => parseFloat(d.AVG_MATH_8_SCORE) || 0),
      text: latestData.map((d) => `${d.STATE}: ${d.AVG_MATH_8_SCORE}`),
      colorscale: "Viridis",
      colorbar: { title: "8th Grade Math Score" },
      name: "8th Grade",
    },
  ];

  Plotly.newPlot("mathScoresMap", mathScoresMapData, {
    title: `Average Math Scores by State (${latestYear})`,
    geo: {
      scope: "usa",
      showlakes: true,
